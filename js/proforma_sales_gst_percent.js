...
$(document).ready(function (e) {
	...
	$.ajax({
		url: BaseUrl+"proforma_sales_percentage/temp_store_percentage",
		type: "POST",
...
$(document).on('click', ".edit_row", function(e) {
	...
	var $td= $(Obj).closest('tr').children('td');
	 $.post( BaseUrl+"proforma_sales/temp_store_delete", { iName: $td.eq(1).text(), Pack: '',BatchNO: '',bills:billno,SlNo:$td.eq(13).text()} );
...
$(document).on('click', ".delete_row", function(e) {
	...
	var $td= $(Obj).closest('tr').children('td');
	$('#ItemName').focus();
	$.post( BaseUrl+"proforma_sales/temp_store_delete", { iName: $td.eq(1).text(), Pack: $td.eq(2).text(),BatchNO: $td.eq(5).text(),bills:billno,SlNo:$td.eq(13).text()} );
	$(Obj).closest("tr").remove();
	count=count-1;
	calculate_total();
	$('#ItemName').focus();
		  ...
		  buttons: {
		    success: {
		      ...
		      callback: function() {
		    	  ...
		    	  $.post( BaseUrl+"proforma_sales/temp_store_delete", { iName: $td.eq(1).text(), Pack: '',BatchNO: '',bills:billno} );
		    	    $('#hsn').val($td.eq(0).text());    	  
		    	  	$('#ItemName').val($td.eq(1).text());
		    	  	$('#MRP').val($td.eq(2).text());
		    		$('#discounted_bill_percent').val($td.eq(3).text());
				  ...
				  $.ajax({
					...
					success: function(data)
					{
					   ...
					   if(obj.error)
						...
						{
							 alert(obj.msg);
							 window.open(BaseUrl+'proforma_sales_percentage/print_bill/'+obj.Billno, "_blank", "toolbar=no,scrollbars=yes,resizable=yes,top=500,left=500,width=800,height=600");
