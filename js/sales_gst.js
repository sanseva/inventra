...
$("#inputHeads").on('submit',(function(e) {
	...
	$.ajax({
		...
		success: function(data)
		{
		   if(data)
			{
			  if(data.status == 'utrFail'){
					alert(data.msg);
				} 
				else if(data.status == 'utrreq') {
                    alert(data.msg);
                    $btn.button('reset');
                }
				else { 
			   ...
			   if ($('#pType').is(":checked"))
				   ...
				   ItemName=$('#ItemNameRet').val();
					...
					+'<td><button type="button" class="btn btn-success btn-sm edit_row"><i class="mdi mdi-pencil"></i></button> <button type="button" class="btn btn-danger btn-sm delete_row"><i class="mdi mdi-delete"></i></button></td>'
			   ...
			   if(confirm('Save the bill?'))
				   {
						...
						data:  {
							...
							rev_chrg:$('#rev_chrg').val(),
							Igst_intra:$('#Igst_intra').val(),
							IsServies:$('#IsServies').val()
						},
		   ...
		   var $btn = $("#btnUpdate").hide();
				  ...
				  $.ajax({
					...
					success: function(data)
					{
					    console.log(data);
						var obj = JSON.parse(data);
						 if(obj.status == 'utrFail'){
                            alert(obj.msg);
                            $("#btnUpdate").show();
                        } else 
                        if(obj.status == 'utrreq') {
                            alert(obj.msg);
                            $("#btnUpdate").show();
                        }else 
                        if(obj.error){
