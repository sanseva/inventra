		   ...
		   if(data)
			{
			   ...
			   if ($('#pType').is(":checked"))
				...
				TableData='<tr class="'+trclass+'">'
					...
					+'<td>'+$('#FreeQty').val()+'</td>'
					+'<td>'+$('#BatchNo').val()+'</td>'
					+'<td class="hideANDseek">01/2050</td>'
					...
					+'<td class="hideANDseek" >1</td>'
					+'<td class="hideANDseek" >'+$('#PurchesRate').val()+'</td>'
					+'<td  >'+$('#hsn_code').val()+'</td>'
					+'</tr>';
				if ($('#pType').is(":checked"))
					$('#ItemName').val('');
		      ...
		      callback: function() {
		    	 ...
		    	 //alert($td.eq(19).text());
		    	  if($td.eq(19).text()=="R")
		    		...
		    		$('#Factor').val('1');
		    		$('#PurchesRate').val($td.eq(23).text());
		    		$('#hsn_code').val($td.eq(24).text());
...
function calculate_ind_item(){
	...
	ADD=		$('#ADD').val()==""?0.00:$('#ADD').val();//SGST
	Rate=		$('#Rate').val()==""?0.00:$('#Rate').val();//purchase rate
	ADD2=		$('#ADD2').val()==""?0.00:$('#ADD2').val();//CGST
	EDperUnti=	$('#EDperUnti').val()==""?0.00:$('#EDperUnti').val();//IGST
	DiscountPer=$('#DiscountPer').val()==""?0.00:$('#DiscountPer').val();//DiscountPer
	//discount
	VALUE3=(parseFloat(Qty)*parseFloat(Rate))*parseFloat(DiscountPer)/100;
	
	taxable_value=(parseFloat(Qty)*parseFloat(Rate))-VALUE3;
	
		//vat @% SGST
	VALUE1=taxable_value*parseFloat(ADD)/100;
	//cst  CGST
	VALUE2=taxable_value*parseFloat(ADD2)/100;
	
	//IGST
	TotalEd=taxable_value*parseFloat(EDperUnti)/100;
	
	
		
	AMT=(Qty*Rate);
	
	ActPruRate = (parseFloat(Qty * Rate) + parseFloat(VALUE1) + parseFloat(VALUE2) + parseFloat(TotalEd)-VALUE3) / (parseFloat(Qty)+parseFloat(FreeQty));
...
if(packageType=="R" || packageType=="A")
					   ...
					   if(!obj.error)
						{
						   alert(obj.msg);
						   $btn.button('reset');
						   location.href=BaseUrl+'purchase/purchase_gst';
						}
						else
						{
							 alert(obj.msg);
