...
<body class="font12">
						...
						<td align="right">
							<table>
								<tr align="right">
									...
									<td class="text-left">
										<strong><font style="color:#17478f;font-size:14px;">A One</font> <font style="color:#00aeef;font-size:14px;">Salasar Pvt Ltd</font>,</strong><br>104, Vasudev Chambers, <br>Mulund Link Road,<br>Bhandup West, Mumbai - 400078<br>Telephone No: +91 22 25660141<br>Email: scs@salasarauction.com<br>
						...
						$retFlag=false;
							foreach($products as $item){
								...
								<tr>
									...
									<td align='center' width="200px"><?=$item['flag']=="R"?$item['return_type']."-":'';?><?=$item['ItemName'] .'<br/>Periods  '.date('d.m.Y',strtotime($item['from_date'])) .' To  '. date('d.m.Y',strtotime($item['to_date']))?></td>
									<?php }else{?>
	...
	<!--	<tr><td><small>GSTIN: <?= $this->session->get('GSTIN');?>, PAN: <?= @$this->session->get('PAN');?></small></td></tr>-->
	</table>
	<br/><br/>
	<strong>For <?= $this->session->get('OrgName');?></strong>
	
			              		<br />
			              		<br />
	<br />
	<u><i>Note:- This is an electronic acknowledgment hence it doesn’t required signature.</i></u>
</body>
<script>
window.print();
</script>
